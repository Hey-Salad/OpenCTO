...
export const mapWorkerRun = (run: WorkerCodebaseRun): CodebaseRun => {
  ...
  return {
    ...
    approval: run.approval
      ? {
          required: Boolean(run.approval.required),
          state: run.approval.state,
          reason: run.approval.reason ?? null,
          approvedByUserId: run.approval.approvedByUserId ?? null,
          decidedAt: run.approval.decidedAt ?? null
        }
      : undefined
  };
};
